...
from llama_index.core.response import Response as ChatResponse
...
from llama_index.core.settings import Settings # Keep Settings import
...
service_context_instance: Optional[Settings] = None
...
def initialize_capgate_agent():
    ...
    # Settings is not callable; just use the global Settings object
    service_context_instance = Settings

    # --- Setup Vector DB for RAG ---
    db = chromadb.PersistentClient(path=str(AGENT_KNOWLEDGE_BASE_DIR))
    chroma_collection = db.get_or_create_collection("mcp_capgate_knowledge")
    vector_store = ChromaVectorStore(chroma_collection=chroma_collection)
    storage_context = StorageContext.from_defaults(vector_store=vector_store) # <-- THIS LINE WAS CORRECTED TO vector_store
...
def index_capgate_knowledge():
    ...
    # Define common extensions including HTML for pdoc output
    COMMON_DOC_EXTS = ['.py', '.c', '.h', '.js', '.md', '.txt', '.html', '.htm', '.pdf']

    # Load documents from CapGate source
    ...
    capgate_docs = SimpleDirectoryReader(str(PROJECT_ROOT), recursive=True,
                                        exclude_hidden=True,
                                        required_exts=COMMON_DOC_EXTS).load_data()

    # Load documents from nexusdevtools (which now contains pdoc HTML)
    ...
    os.makedirs(capgate_internal_docs_path, exist_ok=True)
...
def ask_capgate_agent(query_text: str) -> str:
    ...
    global capgate_agent_instance, capgate_query_engine
    ...
    response_iter: Iterator[ChatResponse] = capgate_agent_instance.stream_chat(prompt_with_context)
    full_response = ""
    for r in response_iter:
        if hasattr(r, "delta"):
            ...
            if hasattr(r.message, "content"):
                full_response += r.message.content
            elif hasattr(r.message, "text"):
                full_response += r.message.text
            else:
                logger.warning(f"Unexpected message format: {r.message}")
        else:
            full_response += str(r)
...
def start_capgate_agent_interactive_session():
    ...
    if capgate_agent_instance is None or capgate_query_engine is None:
        ...
        print("Error: MCP Agent not initialized.")
        return
...
if __name__ == "__main__":
    ...
    temp_project_root = Path(__file__).parent.parent.parent.resolve()
    from paths import PROJECT_ROOT, NEXUSDEVTOOLS_ROOT_DIR, AGENT_KNOWLEDGE_BASE_DIR
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "index":
        ...
        print("Usage: python mod_coor.py [index|ask \"query\"|interactive]")
